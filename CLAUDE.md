...
- `Usuario`: um model pra 3 papéis. Cliente sem `barbeariaId`. `senhaHash` nullable (login Google). `atendeComoBarbeiro` (só DONO). `ehChefe` (só um por barbearia, nunca no cookie).
- `Servico`: corte da barbearia (`precoBase`, `duracaoMinutos`, `imagemUrl`). `ServicoBarbeiro`: preço próprio opcional por barbeiro, ou vínculo de exclusividade (regra 5).
- `Disponibilidade`: janela semanal; `@@unique([barbeiroId, diaDaSemana, horaInicio, horaFim])`.
- `Agendamento`: cliente+barbeiro+data/hora, **sem** `servicoId` único (ver `AgendamentoServico`). Status: PENDENTE → CONFIRMADO/RECUSADO → CONCLUIDO (a qualquer momento, sem travar por horário). `precoCobrado`/`duracaoMinutos` congelam a soma no momento da criação. `cancelamentoSolicitadoEm`/`motivoCancelamento` = pedido de cancelamento em aberto. `confirmadoEm` marca a confirmação (usado no relatório de desempenho).
...
5. `POST /api/servicos`: corte criado pelo DONO é da barbearia toda; criado pelo BARBEIRO fica exclusivo dele (via `ServicoBarbeiro`).
...
10. Dono pode atender como barbeiro (`atendeComoBarbeiro`, interruptor via `PATCH /api/perfil`) — passa a valer como barbeiro em disponibilidade, agendamentos, listagem pública. UI em `/admin`, seção própria.
11. Relatório de desempenho da equipe (`/api/relatorio-equipe`) só pra chefe/dono: faturamento, corte mais feito, cancelados, tempo médio até confirmar, período livre de datas.
12. Agendamento com múltiplos cortes (`servicoIds[]`). Fluxo do cliente em `[slug]`: **primeiro escolhe o barbeiro, depois só vê os cortes vinculados a ele** (não o contrário) — WhatsApp do cliente é obrigatório nesse momento (salvo em `Usuario.whatsapp` via `PATCH /api/perfil` antes de criar o agendamento). Barbeiro vê contato do cliente (`email`/`whatsapp`, botão "Ver contato"); corte pode ser excluído (dono qualquer um, barbeiro só o próprio exclusivo). `CONCLUIDO` antes da hora marcada foi liberado por decisão do usuário (não bloqueia mais) — ao concluir, o horário volta a ficar livre. Painel do barbeiro (e do dono-que-atende) tem 3 campos separados: **Cortes agendados** (CONFIRMADO, com botão "Marcar concluído"), **Cortes concluídos** (CONCLUIDO) e **Cortes cancelados** (CANCELADO, com o motivo).
...
- Sem tela de editar corte existente (API já suporta via PATCH, falta UI).
- Login Google só CLIENTE/DONO (ver regra 7); "Entrar" é estrito, "Cadastro" cria conta.
- Sem tela pra editar vínculo `ServicoBarbeiro` depois da criação.
